...
Worked pattern (module = `domain`, from the report). adt-clients `src/core/domain/types.ts` is the source of truth. **Copy only the param interfaces that actually exist in adt-clients** — apply each ADD/DIFF the report lists for the module. Do NOT invent params from a fixed CRUD template: e.g. `domain` has `ICreate/Update/DeleteDomainParams` but **no** `IReadDomainParams` — the interfaces-only `IReadDomainParams` is handled as INTERFACES_ONLY by Task A1/A5b (removed there), not copied here. Example DIFF fix — `IUpdateDomainParams.package_name`:
...
- [ ] **Step 1: Copy the in-scope shared type declarations verbatim** from `src/core/shared/types.ts` into `src/adt/IAdtShared.ts`. Include the consumer-facing ones (`AdtObjectType`/`AdtObjectTypeLower`/`AdtSourceObjectType`, `IObjectReference`, `ISearchObjectsParams`/`ISearchResult`, `IGetSqlQueryParams`, `IGetTableContentsParams`, `IGetDiscoveryParams`, `IGetWhereUsed*Params`, `IWhereUsedReference`/`IWhereUsedListResult`, `IVirtualFolders*`, `IGetPackageHierarchyOptions`/`PackageHierarchy*`/`IPackageHierarchyNode`, `IGetPackageContentsListOptions`/`IPackageContentItem`, `IInactiveObjectsResponse`). Leave any purely-internal shared type local (note it). Add `export type { … } from './adt/IAdtShared'` to `src/index.ts`.
  - **DO NOT re-add `IReadOptions`** — it already exists in interfaces at `src/shared/IReadOptions.ts` and is exported from root. Adding a second declaration would duplicate the export. Instead: A1's same-name diff already covers it; if the adt-clients local `IReadOptions` (in `shared/types.ts`) drifts from the interfaces one, reconcile the **existing** `src/shared/IReadOptions.ts` to adt-clients' shape (verbatim). On the B side, adt-clients re-exports `IReadOptions` from interfaces like any other promoted type.
