...
describe('Class local includes (using BaseTester)', () => {
  ...
  function logSkip(testName: string, definitionKey: string, definitionName: string, reason: string): void {
  ...
  async function ensureParentClassExists(config: ParentClassConfig): Promise<{ success: boolean; reason?: string; created?: boolean }> {
    try {
      const existing = await client.getClass().read({ className: config.className });
      if (existing) {
      ...
      return { success: false, reason: `Failed to ensure parent class '${config.className}': ${error.message}` };
    }
  }
  ...
  const parentClassCreatedMap = new Map<BaseTester<any, any>, { created: boolean; className: string | null }>();
  ...
  beforeAll(async () => {
    try {
      ...
      definitionsTester = new BaseTester(client.getLocalDefinitions() as any, 'Class', 'update_class_local_definitions', 'local_definitions', testsLogger);
      localTypesTester = new BaseTester(client.getLocalTypes() as any, 'Class', 'update_class_local_types', 'local_types', testsLogger);
      localTestClassTester = new BaseTester(client.getLocalTestClass() as any, 'Class', 'update_class_local_testclass', 'local_testclass', testsLogger);
      localMacrosTester = new BaseTester(client.getLocalMacros() as any, 'Class', 'update_class_local_macros', 'local_macros', testsLogger);
        ...
        tester.setup({
          ...
          ensureObjectReady: async (className: string) => {
            if (options?.skipEnsureParentClass?.()) {
              parentClassCreatedMap.set(tester, { created: false, className: null });
              return { success: true };
            }
            ...
            parentClassCreatedMap.set(tester, { created: result.created === true, className: result.created ? className : null });
            return result;
          }
        });
      };

      setupCommon(definitionsTester, 'LocalDefinitions - full workflow', 'definitionsCode');
      setupCommon(localTypesTester, 'LocalTypes - full workflow', 'localTypesCode');
      setupCommon(localTestClassTester, 'LocalTestClass - full workflow', 'testClassCode');
      setupCommon(localMacrosTester, 'LocalMacros - full workflow', 'macrosCode', {
  ...
  describe('LocalDefinitions', () => {
    ...
    afterEach(async () => {
      ...
      if (parentInfo?.created && parentInfo.className) {
        ...
        const skipCleanup = tc?.params?.skip_cleanup !== undefined
        ...
        if (shouldCleanup) {
          try {
            ...
            testsLogger.warn?.(`⚠️ Failed to cleanup parent class ${parentInfo.className}: ${cleanupError.message}`);
          }
        } else {
          testsLogger.info?.(`⚠️ Cleanup skipped (cleanup_after_test=${cleanupAfterTest}, skip_cleanup=${skipCleanup}) - parent class left for analysis: ${parentInfo.className}`);
        }
      }
    });

    it('should execute full workflow for definitions include', async () => {
      const testName = 'Class - LocalDefinitions - full workflow';
      if (!hasConfig || !definitionsTester) {
        logSkip(testName, 'update_class_local_definitions', 'local_definitions', 'No SAP configuration');
        return;
      }
      ...
      const tc = definitionsTester.getTestCase();
      const params = tc?.params || {};
      const updateCode = params.definitionsCode_update ?? params.definitionsCode ?? config?.definitionsCode;
  ...
  describe('LocalTypes', () => {
    ...
    it('should execute full workflow for implementations include (local helper class)', async () => {
      const testName = 'Class - LocalTypes - full workflow';
      if (!hasConfig || !localTypesTester) {
        logSkip(testName, 'update_class_local_types', 'local_types', 'No SAP configuration');
        return;
      }
      ...
      const tc = localTypesTester.getTestCase();
      const params = tc?.params || {};
      const updateCode = params.localTypesCode_update ?? params.localTypesCode ?? config?.localTypesCode;
  ...
  describe('LocalTestClass', () => {
    ...
    it('should execute full workflow for testclasses include (local ABAP Unit tests)', async () => {
      const testName = 'Class - LocalTestClass - full workflow';
      if (!hasConfig || !localTestClassTester) {
        logSkip(testName, 'update_class_local_testclass', 'local_testclass', 'No SAP configuration');
        return;
      }
      ...
      const tc = localTestClassTester.getTestCase();
      const params = tc?.params || {};
      const updateCode = params.testClassCode_update ?? params.testClassCode ?? config?.testClassCode;
  ...
  describe('LocalMacros', () => {
    ...
    it('should execute full workflow for macros include (on-premise only)', async () => {
      if (isCloudSystem) {
        const definition = getTestCaseDefinition('update_class_local_macros', 'local_macros');
        const testName = 'Class - LocalMacros - full workflow';
        logBuilderTestStart(testsLogger, testName, definition);
        logBuilderTestSkip(testsLogger, testName, 'Macros are not supported in cloud systems (BTP ABAP Environment)');
        logBuilderTestEnd(testsLogger, testName);
        return;
      }
      ...
      if (!hasConfig || !localMacrosTester) {
        logSkip(testName, 'update_class_local_macros', 'local_macros', 'No SAP configuration');
        return;
      }
      ...
      const existing = await client.getLocalMacros().read({ className: config.className }, 'active');
      if (!existing) {
        const definition = getTestCaseDefinition('update_class_local_macros', 'local_macros');
        logBuilderTestStart(testsLogger, testName, definition);
        logBuilderTestSkip(testsLogger, testName, 'Macros include is not available in this system');
        logBuilderTestEnd(testsLogger, testName);
        return;
      }
