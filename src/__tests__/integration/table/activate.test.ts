/**
 * Unit test for Table activation
 * Tests activateTable function
 *
 * Enable debug logs: DEBUG_TESTS=true npm test -- unit/table/activate.test
 */
...
describe('Table - Activate', () => {
  ...
  it('should activate table', async () => {
    if (!hasConfig) {
      logger.warn('⚠️ Skipping test: No .env file or SAP configuration found');
      return;
    }

    const testCase = getEnabledTestCase('activate_table');
    if (!testCase) {
    ...
    try {
      validateTestCaseForUserSpace(testCase, 'activate_table');
    } catch (error: any) {
    ...
    const response = await activateTable(
      connection,
