...
describe('Function Module - Delete', () => {
  ...
  it('should delete function module', async () => {
    ...
    try {
      ...
      console.log(`ℹ️  Created FM ${functionModuleName} for delete test`);
    } catch (error: any) {
      if (error.message.includes('already exists')) {
        console.log(`ℹ️  FM ${functionModuleName} already exists`);
      } else if (error.message.includes('S_ABPLNGVS')) {
        console.warn(`⚠️  Skipping delete test: Missing S_ABPLNGVS authorization to create FM`);
        return;
      } else {
        throw error;
      }
    ...
    try {
      ...
      console.log(`✅ Verified FM deletion`);
    }
  }, 30000);
});
