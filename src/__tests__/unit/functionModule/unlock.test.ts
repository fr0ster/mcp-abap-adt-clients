/**
 * Unit test for FunctionModule unlocking
 * Tests unlockFunctionModule function
 *
 * Enable debug logs: DEBUG_TESTS=true npm test -- unit/functionModule/unlock.test
 */
...
describe('FunctionModule - Unlock', () => {
  ...
  it('should unlock function module', async () => {
    ...
    const testCase = getEnabledTestCase('unlock_function_module');
    if (!testCase) {
    ...
    try {
      validateTestCaseForUserSpace(testCase, 'unlock_function_module');
    } catch (error: any) {
    ...
    // First lock the function module to get a lock handle
    ...
    logger.debug(`✅ Unlocked function module: ${testCase.params.function_module_name}`);
  }, 30000);
});
