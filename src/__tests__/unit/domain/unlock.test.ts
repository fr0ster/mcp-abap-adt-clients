/**
 * Unit test for Domain unlocking
 * Tests unlockDomain function
 *
 * Enable debug logs: DEBUG_TESTS=true npm test -- unit/domain/unlock.test
 *
...
describe('Domain - Unlock', () => {
  ...
  it('should unlock domain', async () => {
    ...
    const testCase = getEnabledTestCase('unlock_domain', 'test_domain');
    if (!testCase) {
    ...
    try {
      validateTestCaseForUserSpace(testCase, 'unlock_domain');
    } catch (error: any) {
    ...
    // First lock the domain to get a lock handle
    ...
    // Now unlock it
    const response = await unlockDomain(
      connection,
      testCase.params.domain_name,
      lockHandle,
      sessionId
