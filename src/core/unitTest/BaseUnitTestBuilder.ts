...
export abstract class BaseUnitTestBuilder extends ClassBuilder {
  ...
  ): Promise<IClassBuilderConfig | undefined> {
    try {
      this.logger?.info(
        `'Reading test class source:'  this.config.className  'version:' ${`version`}`,
      );
      ...
      this.logger?.info(
        `'Test class source read successfully:'  result.status  'bytes:' ${`result.data?.length || 0`}`,
      );
  ...
  async update(
    sourceCode?: string,
    _options?: { implementations?: string; testClasses?: string },
  ): Promise<this> {
    try {
      if (!this.lockHandle) {
        throw new Error(
   ...
   */
  async check(
    _version: 'active' | 'inactive' = 'inactive',
    sourceCode?: string,
  ): Promise<any> {
    try {
  ...
  async getStatus(withLongPolling: boolean = true): Promise<this> {
    if (!this.unitTestState.runId) {
      throw new Error(
        'Run ID is required. Call runForClass() or runForObject() first.',
      );
    }
    try {
      this.logger?.info(
        `'Getting unit test status  runId:' ${`this.unitTestState.runId`}`,
      );
      const response = await getClassUnitTestStatus(
        this.connection,
        this.unitTestState.runId,
        withLongPolling,
      );
  ...
  async getResult(options?: {
    ...
    try {
      this.logger?.info(
        `'Getting unit test result  runId:' ${`this.unitTestState.runId`}`,
      );
