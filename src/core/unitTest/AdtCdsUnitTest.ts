...
export class AdtCdsUnitTest extends AdtUnitTest {
   ...
   */
  async validate(config: Partial<ICdsUnitTestConfig>): Promise<ICdsUnitTestState> {
    ...
    if (config.cdsViewName) {
      try {
        ...
        const response = await validateCdsForUnitTest(this.connection, config.cdsViewName);
   ...
   */
  async delete(config: Partial<ICdsUnitTestConfig>): Promise<ICdsUnitTestState> {
