    ...
    >
{
  ...
  private name(config: Partial<IAccessControlConfig>): string {
    return config.accessControlName as string;
  }
  ...
  async create<E extends IAdtError = IAdtError>(
    config: Omit<IAccessControlConfig, 'source'> & { source?: never },
  ...
  async check<E extends IAdtError = IAdtError>(
    ...
    return answering(
      () => checkAccessControl(connection, name, version, config.source),
    ...
    const answer = await answering(
      () =>
        inStatefulSession(this.connection, () =>
          lockAccessControl(this.connection, name),
        ),
      lockHandleOf,
      options?.analyse,
    );
  ...
  async getVersions<E extends IAdtError = IAdtError>(
    ...
    return answering(
      () => getAccessControlVersions(this.connection, config),
  ...
  async getVersionSource<E extends IAdtError = IAdtError>(
    ...
    return answering(
      () => getAccessControlVersionSource(this.connection, contentUri),
