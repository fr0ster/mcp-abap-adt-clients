...
export class DataElementBuilder implements IBuilder<IDataElementState> {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        ...
        if (
          ...
          (errorText.toLowerCase().includes('resource') &&
            errorText.toLowerCase().includes('exist'))
        ) {
          this.logger?.warn(
            `Data element already exists, validation skipped: ${this.config.dataElementName}`,
          );
      ...
      if (
        ...
        (errorMsg.toLowerCase().includes('object type') &&
          errorMsg.toLowerCase().includes('not supported'))
      ) {
        this.logger?.warn(
          `'Validation not supported for DTEL/DE in this SAP system  skipping:' ${`this.config.dataElementName`}`,
        );
        if (error.response) {
          this.state.validationResponse = error.response;
          return error.response;
  ...
  async create(): Promise<this> {
    try {
      ...
      if (!this.config.typeKind) {
        throw new Error(
          'typeKind is required in DataElementBuilderConfig. Must be one of: domain, predefinedAbapType, refToPredefinedAbapType, refToDictionaryType, refToClifType',
        );
      }
      ...
      if (
        ...
        if (!this.config.dataType) {
          throw new Error(
            `dataType is required when typeKind is '${typeKind}'. Provide data type (e.g., CHAR, NUMC, INT4).`,
          );
        }
      } else {
        ...
        if (typeKind === 'domain') {
          ...
          if (!this.config.typeName && !this.config.dataType) {
            throw new Error(
              `typeName (domain name) is required when typeKind is 'domain'. Provide domain name (e.g., ZOK_AUTH_ID).`,
            );
          }
        } else {
          ...
          if (!this.config.typeName) {
            throw new Error(
              `typeName is required when typeKind is '${typeKind}'. Provide ${typeKind === 'refToDictionaryType' ? 'data element name' : 'class name'}.`,
            );
          }
        }
      }
  ...
  async update(): Promise<this> {
    try {
      if (!this.lockHandle) {
        throw new Error(
          'Data element must be locked before update. Call lock() first.',
        );
      }
      if (!this.config.packageName) {
        throw new Error('Package name is required');
      ...
      const username =
        process.env.SAP_USER || process.env.SAP_USERNAME || 'MPCUSER';
  ...
  async check(
    ...
    try {
      this.logger?.info(
        `Checking data element: ${this.config.dataElementName}, version: ${version}`,
      );
      ...
      const errorMsg = error.message || '';
      if (
        errorMsg.toLowerCase().includes('importing') &&
        errorMsg.toLowerCase().includes('database')
      ) {
        this.logger?.warn(
          `'Check not fully supported for data element (common for DDIC objects)  continuing:' ${`this.config.dataElementName`}`,
        );
  ...
  private parseDataElementXml(xmlData: string): IDataElementConfig | undefined {
    try {
      ...
      return {
        ...
        length: dtel['dtel:dataTypeLength']
          ? parseInt(dtel['dtel:dataTypeLength'], 10)
          : undefined,
        decimals: dtel['dtel:dataTypeDecimals']
          ? parseInt(dtel['dtel:dataTypeDecimals'], 10)
          : undefined,
