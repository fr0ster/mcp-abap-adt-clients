...
- **`ENHANCEMENT_TYPE_CODES` is now actually exported.** The README listed it among this package's runtime exports; it was defined in `core/enhancement/types.ts`, re-exported by that module's barrel, and never reached the root. It is ours, the docs promise it, so it is now handed out. The docs-import test did not catch this one: the README mentions it in prose rather than in an `import` statement, and the test checks imports.
