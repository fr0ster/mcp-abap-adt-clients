...
### Changed
- Requires `@mcp-abap-adt/interfaces` **^11.6.0** — 11.5.0 for the connection capability atoms this release consumes, then 11.6.0 to keep the declared floor level with the published contract (its addition, the authorization-strategy contract, is not used here). The dev-time connector moves to **^2.0.0**.
