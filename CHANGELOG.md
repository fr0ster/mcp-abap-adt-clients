...
Requires `@mcp-abap-adt/interfaces@^38.0.0`.
...
- **This package is now `LGPL-3.0-only`.** It was MIT through 16.x, and those
  versions stay MIT — a licence change is not retroactive, and anyone already
  using 16.x under MIT keeps that grant for 16.x.
...
- **`AxiosResponse` is called `IAdtResponse` throughout.** The type is `IAdtResponse` from `@mcp-abap-adt/interfaces`; 265 TypeScript files imported it as `AxiosResponse` and used that name in 933 places, and four JSDoc blocks in a `.js` test helper described ADT responses by it. The name was a leftover from when axios was a direct dependency, and it had become a lie: responses arrive over HTTP *or* RFC, and a reader of `Promise<AxiosResponse>` in an RFC path had to know it did not mean what it said. Internal naming only — no exported name changes, and the public surface is byte-for-byte the same. `AxiosError`, which really is axios's, is untouched.
- **This package no longer re-exports types owned by `@mcp-abap-adt/interfaces` — all 145 of them.** That was **more than half** of a 249-name public surface, which is now 108. A consumer could hold `IClassConfig`, `IPackageState` or even `IAbapConnection` believing it came from this client, and see a type appear to change whenever this client released, for reasons that had nothing to do with it. Import them from the package that owns them:
  ...
  **One of the 145 is a runtime value, not a type: `SERVICE_BINDING_VARIANT_MAP`.** It is defined in `@mcp-abap-adt/interfaces` and was republished from here, so it goes with the rest — but unlike the types, a `import { SERVICE_BINDING_VARIANT_MAP } from '@mcp-abap-adt/adt-clients'` fails at runtime, not just at compile time. Import it from `@mcp-abap-adt/interfaces`. `resolveBindingVariant` beside it is genuinely this package's own and stays.
  ...
  ```ts
  // before
  import type { PackageHierarchyNode, ObjectReference } from '@mcp-abap-adt/adt-clients';
  // after
  import type { IPackageHierarchyNode, IObjectReference } from '@mcp-abap-adt/interfaces';
  ```

  The aliases were a temporary measure that outlived its purpose. Thirteen of the eighteen appeared **only** in this package's own barrel — they existed to be re-exported and for nothing else; five were referenced nowhere at all; no production file used any of them. Two integration tests did, and now import from `interfaces` like any other consumer. Public surface: 266 exported names down to 249, with the 45 runtime exports unchanged.
