...
- **The surface guard now covers prose, not just imports.** Both defects that reached review — a promised `ENHANCEMENT_TYPE_CODES` that was never exported, and `SERVICE_BINDING_VARIANT_MAP` removed while still documented as local — sat in a bulleted list of values rather than in an `import` statement, so the import-checking test could not see them. `publicApiSurface.test.ts` now asserts that every value the README *names* as an export exists. Telling a value from a type in prose is tractable only because of the naming rule: an interface is `I` followed by a capital.
