...
- `TraceDocumentError`. A trace document that is empty, unparseable, or rooted
  ...
  The check runs at **four** levels, because a wrong guess hides at every one:
  ...
  property not being there, and anything else is present. A document understood
  at all four levels may still be empty, and then it reads as empty.
