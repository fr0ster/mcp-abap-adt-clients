...
const YAML_CONFIG = path.resolve(
  __dirname,
  '../src/__tests__/helpers/test-config.yaml',
);
...
const READ_STANDARD_PATTERNS = [/read standard/i, /read.*standard/i];

const READ_TRANSPORT_PATTERNS = [/read transport/i, /transport request/i];
...
function verifyBuilderTest(filePath) {
  ...
  const readStandardTests = classified.filter(
    (t) => t.type === 'read_standard',
  );
  const readTransportTests = classified.filter(
    (t) => t.type === 'read_transport',
  );
...
function main() {
  ...
  for (const result of results) {
    ...
    console.log(
      `   Tests: ${result.testCount} (expected: ${result.expectedCount})`,
    );
