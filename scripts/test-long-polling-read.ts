...
async function testLongPollingRead() {
  try {
    ...
    if (!testCase) {
      ...
      console.error(
        'Please enable create_domain test case in test-config.yaml',
      );
      process.exit(1);
    }
    ...
    const packageCheck = ensurePackageConfig(
      testCase.params || {},
      'Long polling read test',
    );
    if (!packageCheck.success) {
      console.error(
        `❌ ${packageCheck.reason || 'Package name is not configured'}`,
      );
      console.error(
        'Please set package_name in test-config.yaml or environment.default_package',
      );
      process.exit(1);
    }
