...
import { objectVersions } from '../results/versions';

/**
 * The version-history reading, moved with its tests from adt-clients
 * (`versionsParse.test.ts`), where `getVersions` applied it for every caller.
 * The feeds are real ones — a table's, and a program's carrying a transport.
 */
...
describe('objectVersions', () => {
  it('parses a single-entry feed', () => {
    const v = objectVersions(answer(FEED));
    expect(v).toHaveLength(1);
    expect(v[0]).toEqual({
      transportRequest: undefined,
      transportDescription: undefined,
      versionId: '00000',
      author: 'CB9980008038',
...
it('an empty answer is an empty history, not an error', () => {
  expect(objectVersions(answer(''))).toEqual([]);
});
